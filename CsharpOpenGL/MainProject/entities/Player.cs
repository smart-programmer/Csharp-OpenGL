...
namespace MainProject.entities 
{
    public class Player : Entity
    {
        ...
        {
            checkInputs(window);
            increaseRotation(0, currentTurnSpeed * MainClass.getFrameTimeSeconds(), 0);
            float distance = currentSpeed * MainClass.getFrameTimeSeconds();
            float dx = (float) (distance * Math.Sin(math.toRadians(rotY)));
            float dz = (float)(distance * Math.Cos(math.toRadians(rotY)));
            increasePosition(-dx, 0, dz);
            upwardsSpeed += GRAVITY * MainClass.getFrameTimeSeconds();
            increasePosition(0, upwardsSpeed * MainClass.getFrameTimeSeconds(), 0);
